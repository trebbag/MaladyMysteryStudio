...
- An OpenAI project/account with sufficient billing/quota enabled for the chosen `MMS_MODEL`
  ...
  - Current blocker as of March 7, 2026: a real-key v2 quality validation run (`pneumococcal-pneumonia-hnil8lsf`) failed at `KB0` with `429 You exceeded your current quota`, so additional live validation is blocked until quota is restored.
